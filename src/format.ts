...
export const getSignaturesCachePath = () =>
  join(homedir(), ".foundry", "cache", "signatures");

export const loadSignaturesCache = (): SignaturesCache => {
  try {
    return JSON.parse(
      readFileSync(getSignaturesCachePath(), { encoding: "utf8" })
    );
  } catch {}
  ...
  switch (typeof arg) {
    case "object": {
      ...
      const formattedObj = Object.entries(arg)
        .map(
          ([key, value]) =>
            `\n${getIndentLevel(level + 1)}${key}: ${grey(
              formatArg(value, level + 1, config)
            )},`
        )
        .join("");
...
export const formatCallLog = (
  ...
  return `${getIndentLevel(level + 1, true)}${yellow("LOG")} ${eventName}(${
    formattedArgs ?? ""
  })`;
};
...
export const formatCallTrace = (
  ...
  return `${
    ...
    trace.logs
      ? `\n${trace.logs.map((log) =>
          formatCallLog(log, level, signatures, config)
        )}`
      : ""
  }
...
export async function formatFullTrace(
  ...
  if (unknownFunctionSelectors || unknownEventSelectors) {
    ...
    if (lookup.ok) {
      Object.entries<{ name: string; filtered: boolean }[] | null>(
